...
const SurvivorForm: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await createSurvivor({
        ...
        inventory: Object.entries(formData.inventory).map(
          ([item, quantity]) => ({
  ...
  return (
    <Box sx={{ maxWidth: 400, mx: 'auto', mt: 4 }}>
      ...
      {success && (
        <Alert severity="success">Survivor successfully created!</Alert>
      )}
      {inventoryError && (
        ...
        {Object.keys(formData.inventory).length > 0 && (
          <Box mt={2}>
            ...
            {Object.entries(formData.inventory).map(([item, quantity]) => (
              <Typography key={item}>
                {inventoryItems.find((inv) => inv.name === item)?.icon} {item}:{' '}
                {quantity}
              </Typography>
            ))}
          </Box>
          ...
          {inventoryItems.map(({ name, icon }) => (
            ...
            >
              ...
              <Box display="flex" alignItems="center">
                <IconButton
                  onClick={() => handleInventoryChange(name, -1)}
                  disabled={!formData.inventory[name]}
                >
                  <RemoveIcon />
                </IconButton>
                <TextField
